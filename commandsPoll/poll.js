...
module.exports = {
  ...
  run: async (message, _, __, removeClient) => {
    ...
    */
    ...
    if (command.length <= 1) {
      if (message.attachments.size < 1 ) {
        ...
        command = await parseAttachments(message.attachments.first());
    ...
    if (errorRoles.length > 0) {
      message.channel.send(
        `Te estoy vigilando ${message.author.toString()}... ¿En serio pensabas que me puedes engañar? ESTOS ROLES NO EXISTEN: ${errorRoles.join(
          ', '
        )}`
      );
      return;
    ...
    if (!parseExpiration(nextId.toString(), poll, groups[2])) {
      message.channel.send(
        'Oops, parece que algo raro me ha ocurrido... Iré a quejarme a shu un rato :heart_eyes:, vuelve a introducir el comando mientras tanto'
      );
      return;
    }
    ...
    message.channel.send(
      `¡Encuesta creada con éxito! Usa /v ${nextId} "la opción que mas te guste sin las comillas" para votar en ella. Si quieres ver las opciones y los resultados, solo si están visibles, usa /v ${nextId} únicamente... ¿Quieres algo más o me vas a tener aquí esperando todo el día? ¡VOTA!`
    );
  }
};
...
async function expiredPoll(pollId) {
  ...
  var results = new Map(
    [...Object.entries(poll.options)].sort((a, b) => b[1] - a[1])
  );
  var message = '';
  var i = 1;
  ...
  user.send(
    `Encuesta ${poll.title} finalizada con los siguientes resultados:\n${message}`
  );
  polls.delete(pollId);
}
...
async function displayHelp(message, error = undefined){
  if (error != undefined) message.channel.send(error);

  let prefix = dbprefix.has(`${message.guild.id}`) ? await dbprefix.get(`${message.guild.id}`) : botconfig.prefix;
